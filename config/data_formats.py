...
KEY_TT_DEG          = 'turntable_deg'        # 턴테이블 각도
KEY_TT_FEED         = 'turntable_feed_rate'  # 턴테이블 속도
KEY_POLAR_THETA     = 'polar_coord_theta'    # 극좌표 각도 (턴테이블 연동)
...
TXT_SCHEMA: dict[str, dict] = {
    ...
    'Y': {'name': KEY_LEGACY_Y,      'type': float},        # Y 좌표 (mm)
    'Z': {'name': KEY_LEGACY_Z,      'type': float},        # Z 좌표 (mm)
    'W': {'name': 'yaw',             'type': float},        # Yaw(회전) 각도 (deg)
    'P': {'name': 'pitch',           'type': float},        # Pitch(상하 기울기) 각도 (deg)
    'R': {'name': 'roll',            'type': float},        # Roll(좌우 비틀기) 각도 (deg)
    'M': {'name': KEY_TOOL_ROT_RPM,  'type': float},        # 툴 자전 속도 (rpm)
    'N': {'name': KEY_TOOL_REV_RPM,  'type': float}         # 툴 공전 속도 (rpm)
}
...
FANUC_SCHEMA: dict[str, dict] = { 
    ...
    'Z': {'source': KEY_Z,  'type': float},
    'W': {'source': KEY_W,  'type': float},
    'P': {'source': KEY_P,  'type': float},
    'R': {'source': KEY_R,  'type': float}
}
...
ROBOT_KEYS = {
    KEY_X, KEY_Y, KEY_Z, KEY_W, KEY_P, KEY_R,
