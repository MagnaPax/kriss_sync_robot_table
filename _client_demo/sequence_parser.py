...
def parse_txt_to_sequence(raw_lines: List[str]) -> SequenceDict:
    """
    FANUC 좌표 TXT 파일의 raw 문자열 리스트를 의미있는 딕셔너리 시퀀스로 파싱합니다.

    인자:
        raw_lines (List[str]): 각 줄이 '값 값 값...' 형태인 문자열 리스트 (공백 분리)
...
def parse_csv_to_sequence(raw_sequence: List[List[str]]) -> SequenceDict:
    """
    ...
    인자:
        raw_sequence (List[List[str]]): CSV 로더에서 반환된 형태
