...
2. PositionModel.parse_positions_from_data()
    - 성공 케이스 (정상 데이터)
...
# 테스트 대상 모듈 임포트
from models.position_model import Position, PositionModel
...
def sample_data() -> Dict[str, Any]:
    """[Happy Path] PositionModel의 main 테스트에서 가져온 정상 샘플 데이터"""
    return {
        "Macro_1": {
            "macro_id": "Macro_1", 
            "name": "매크로 1 이름", 
...
def parsed_positions(sample_data: Dict[str, Any]) -> Dict[str, Position]:
    ...
    return PositionModel.parse_positions_from_data(sample_data)
...
# --- 2. PositionModel.parse_positions_from_data() 테스트 ---

def test_parse_success(sample_data: Dict[str, Any]):
    """[성공] 정상적인 데이터 파싱을 테스트합니다."""
    positions = PositionModel.parse_positions_from_data(sample_data)
...
def test_parse_fail_not_a_dict():
    ...
    with pytest.raises(TypeError, match="dict 형식이어야 합니다"):
        PositionModel.parse_positions_from_data(invalid_data) # type: ignore
...
def test_parse_fail_inner_data_not_dict(sample_data: Dict[str, Any]):
    ...
    with pytest.raises(ValueError, match="데이터가 dict가 아닙니다"):
        PositionModel.parse_positions_from_data(sample_data)
...
def test_parse_fail_empty_dict():
    ...
    with pytest.raises(ValueError, match="로드된 매크로가 하나도 없습니다"):
        PositionModel.parse_positions_from_data({})
...
def test_parse_fail_missing_key():
    ...
    with pytest.raises(KeyError, match="필수 키 'x'가 없습니다"):
        PositionModel.parse_positions_from_data(invalid_data)
...
def test_get_position_fail_keyerror(parsed_positions: Dict[str, Position]):
    ...
    with pytest.raises(KeyError, match="찾을 수 없습니다"):
        PositionModel.get_position(parsed_positions, "Macro_99")