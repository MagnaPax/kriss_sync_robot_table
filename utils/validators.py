...
class NumericValidator(QDoubleValidator):
    ...
    """
    def __init__(self, error_callback=None, parent=None):
        super().__init__(parent)
        self.error_callback = error_callback
        self.setNotation(QDoubleValidator.Notation.StandardNotation)

    def validate(self, input: str, pos):    # type: ignore[override]
        # 입력 중인 상태 허용
        if input in ["", "-", ".", "-."]:
...
class PatternValidator(QRegularExpressionValidator):
    ...
    """
    def __init__(self, pattern: str, error_msg: str, error_callback=None, parent=None):
